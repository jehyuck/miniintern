...
CREATE TABLE "mclass" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "chk_mclass_time_order" CHECK ("mclass"."end_date" > "mclass"."start_date"),
	CONSTRAINT "chk_mclass_capacity_positive" CHECK ("mclass"."capacity" > 0)
);
--> statement-breakpoint
CREATE TABLE "users" (
...
CREATE UNIQUE INDEX "uniq_active_application" ON "application" USING btree ("user_id","mclass_id") WHERE status = 'APPLIED';--> statement-breakpoint
...
CREATE INDEX "idx_mclass_user" ON "mclass" USING btree ("user_id");--> statement-breakpoint
CREATE UNIQUE INDEX "userId_title_unique" ON "mclass" USING btree ("user_id","title");